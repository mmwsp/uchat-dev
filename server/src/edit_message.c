...
void edit_message(int socket_fd, json_object *json_received_obj) {
    ...
    char *edited_text = json_object_get_string(jedited_text);
    ...
    json_object_object_add(json_obj, "is_edited", json_object_new_string(1));
    
    broadcast_connection(json_received_obj, socket_fd, json_obj);
}
