...
void join_chat(int socket_fd, json_object *json_received_obj) {
    ...
    if(chat_type == 1) {
        ...
        int join_type = 1; 
        int cus_type = 3;
        const char *sender_bio;
        const char *sender_name;
        int sender_online;
        const char *user2_name;
        const char *user2_bio;
        int user2_online;
        sqlite3 *db;
        ...
        if (rc == SQLITE_ROW) {
            sender_name = (const char *)sqlite3_column_text(stmt, 0);
            sender_bio = (const char *)sqlite3_column_text(stmt, 1);
        ...
        rc = sqlite3_bind_int(stmt, 1, user2_id);
        if (rc != SQLITE_OK) {
            fprintf(stderr, "Error binding parameter: %s\n", sqlite3_errmsg(db));
            sqlite3_finalize(stmt);
            sqlite3_close(db);
        ...
        rc = sqlite3_step(stmt);
        if (rc == SQLITE_ROW) {
            user2_name = (const char *)sqlite3_column_text(stmt, 0);
            user2_bio = (const char *)sqlite3_column_text(stmt, 1);
            user2_online = sqlite3_column_int(stmt, 2);
        } else {
        ...
        if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) == SQLITE_OK) {
            ...
            if (sqlite3_step(stmt) == SQLITE_ROW) {
                ...
                struct json_object *json_chat_obj = json_object_new_object();
                json_object_object_add(json_chat_obj, "method", json_object_new_string("join_chat"));
                json_object_object_add(json_chat_obj, "c_type", json_object_new_int(2));
                json_object_object_add(json_chat_obj, "chat_id", json_object_new_int(chat_id));
                ...
                const char *json_str = json_object_to_json_string(json_chat_obj);
